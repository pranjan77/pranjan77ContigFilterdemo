...
class pranjan77ContigFilterdemoTest(unittest.TestCase):

    @classmethod
    def setUpClass(cls):
        token = environ.get('KB_AUTH_TOKEN', None)
        cls.ctx = {'token': token, 'provenance': [{'service': 'pranjan77ContigFilterdemo',
        ...
        for nameval in config.items('pranjan77ContigFilterdemo'):
            cls.cfg[nameval[0]] = nameval[1]
        cls.wsURL = cls.cfg['workspace-url']
        cls.wsClient = workspaceService(cls.wsURL, token=token)
        cls.serviceImpl = pranjan77ContigFilterdemo(cls.cfg)
    ...
    def getWsName(self):
        ...
        wsName = "test_pranjan77ContigFilterdemo_" + str(suffix)
